...
class EventManager {
     ...
     * @param funObj - 响应对象
     */
    public addEvent(type: number | string, func: Function, funObj: Component): void {
        let eventMap = this._eventMap;
        let typeMap = this._typeMap;

        let hashCode = funObj.uuid != null ? funObj.uuid : null; //获取对象的唯一id
        let key = hashCode;
     ...
     * @param funObj - 响应对象
     */
    public removeEvent(type: number | string, func: Function, funObj: Component): void {
        if (!funObj) return;
        let key = funObj.uuid != null ? funObj.uuid : null;
     ...
     * @param evtObj -
     */
    public removeEvents(evtObj: Component): void {
        let eventMap = this._eventMap;
        let keyV = evtObj.uuid != null ? evtObj.uuid : null;
